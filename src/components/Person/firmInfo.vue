...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (valid) {
      ruleForm.period = formatYearRange(ruleForm.period as unknown as [Date, Date]);
      // 等待 axios 请求完成（添加 await）
      const response = await axios.post('http://localhost:8090/job/update', ruleForm,{
        headers: {
...
async function getUserInfo() {
  ...
  const res = await axios.post("http://localhost:8090/job/getInfo", null, {
...
<template>
  ...
  >
    ...
    <el-form-item label="薪资待遇" prop="salaryRange">
      <el-input v-model="ruleForm.salaryRange" placeholder="请输入职业薪资水平"/>
    </el-form-item>
